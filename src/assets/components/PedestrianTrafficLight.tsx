...
import { pedestrianTrafficLightMachine } from "../machines/pedestrianTrafficLightMachine";

function PedestrianTrafficLight() {
  const [current, send] = useMachine(pedestrianTrafficLightMachine, {
    devTools: true
  });

  // const checkRedState = () => {
  //   console.log(pedestrianStates);
  // };

  // const checkRedState = () => {
  //   console.log(pedestrianTrafficLightMachine.transition("red", { type: "SWITCH_LIGHT" }).value);
  // };

  return (
    <div className="grid grid-flow-row gap-5">
      <h1 className="text-center">PedestrianTrafficLight</h1>
      {/* <ul className="text-center">Current state:
        {Object.keys(current.value).map((stateKey) => (
          <li key={stateKey}>
      "{stateKey}": "{current.value[stateKey]}"
          </li>
        ))}
      </ul> */}
